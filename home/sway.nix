...
let
  ...
  config = ''
    ...
    bindsym $mod+o exec ${alacritty}/bin/alacritty -e bash -c 'vim $(fd . $HOME --hidden | fzf)'
    ...
    bindsym $mod+p exec ${alacritty}/bin/alacritty --class launcher -e bash -c 'compgen -c | sort -u | fzf --no-extended --print-query | tail -n1 | xargs -r ${swaymsgPath} -t command exec'
